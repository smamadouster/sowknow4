...
from app.services.synthesis_service import SynthesisRequest, synthesis_service
...
async def graph_aware_answer(
    ...
    answer_stream = await graph_rag_service.generate_graph_aware_answer(
        query=query, enhanced_results=enhanced, db=db, stream=False
    )
...
async def find_entity_paths(
    ...
    paths = await graph_rag_service.find_entity_paths(
        source_entity_name=source, target_entity_name=target, db=db, max_path_length=max_length
    )
...
async def get_entity_neighborhood(
    ...
    """
    neighborhood = await graph_rag_service.get_entity_neighborhood(entity_name=entity_name, db=db, radius=radius)
...
async def synthesize_documents(
    ...
    confidential_docs = [
        {"id": str(doc.id), "filename": doc.filename} for doc in documents if doc.bucket == DocumentBucket.CONFIDENTIAL
    ]
    ...
    result = await synthesis_service.synthesize(request=request, db=db)
...
async def reason_about_event(
    ...
    reasoning = await temporal_reasoning_service.reason_about_temporal_relationships(
        event_id=str(event_id), db=db, time_window_days=time_window_days
    )
...
async def analyze_entity_evolution(
    ...
    evolution = await temporal_reasoning_service.analyze_evolution(
        entity_name=entity_name, db=db, time_months=time_months
    )
...
async def find_temporal_patterns(
    ...
    """
    patterns = await temporal_reasoning_service.find_temporal_patterns(db=db, min_occurrences=min_occurrences)
...
async def reveal_entity(
    ...
    entity_info = await progressive_revelation_service.reveal_entity_info(
        entity_id=str(entity_id), user=current_user, layer=layer, db=db
    )
...
async def get_family_context(
    ...
    family_context = await progressive_revelation_service.generate_family_context(
        focus_person=focus_person, db=db, depth=depth, include_timeline=include_timeline
    )
