...
from app.models.knowledge_graph import Entity, EntityType, TimelineEvent
...
class TimelineConstructionService:
    ...
    async def build_document_timeline(self, document_id: str, db: AsyncSession) -> list[dict[str, Any]]:
        """
    ...
    async def build_entity_timeline(self, entity_name: str, db: AsyncSession) -> list[dict[str, Any]]:
        """
        ...
        """
        # Find entity
        entity = (await db.execute(select(Entity).where(Entity.name.ilike(f"%{entity_name}%")))).scalar_one_or_none()
        ...
        events = (
            (
                await db.execute(
                    select(TimelineEvent).where(TimelineEvent.id.in_(event_ids)).order_by(TimelineEvent.event_date)
                )
            )
            .scalars()
            .all()
    ...
    async def detect_evolution_patterns(
        ...
        mentions = (
            (await db.execute(select(EntityMention).where(EntityMention.entity_id.in_(entity_ids)))).scalars().all()
        )
        ...
        return {
            ...
            "trend": trend,
            "stages": stages,
            "timeline": [{"month": month, "count": count} for month, count in sorted_mentions],
        }

    def _identify_evolution_stages(self, monthly_mentions: list[tuple]) -> list[dict[str, Any]]:
        """
        ...
        for i, (_month, count) in enumerate(monthly_mentions):
                    ...
                    stages.append(
                        {
                            ...
                            "average_count": sum(c for _, c in monthly_mentions[current_stage_start:i])
            ...
            stages.append(
                {
                    ...
                    "average_count": sum(c for _, c in monthly_mentions[current_stage_start:])
    ...
    async def get_timeline_for_period(self, start_date: date, end_date: date, db: AsyncSession) -> list[dict[str, Any]]:
        """
    ...
    async def suggest_timeline_insights(self, db: AsyncSession, limit: int = 10) -> list[dict[str, Any]]:
        """
        ...
        top_entities = (
            (await db.execute(select(Entity).order_by(Entity.relationship_count.desc()).limit(20))).scalars().all()
        )
                ...
                insights.append(
                    {
                        ...
                        "span_days": (
                            (last.event_date - first.event_date).days if first.event_date and last.event_date else 0
                        ),
