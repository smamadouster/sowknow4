...
def fetch_sample(conn, n: int) -> list[dict]:
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute(
            """
            SELECT id, chunk_text AS content, embedding_vector AS embedding
            FROM sowknow.document_chunks
            WHERE embedding_vector IS NOT NULL
              AND length(chunk_text) BETWEEN 50 AND 4000
            ORDER BY random()
            LIMIT %s
            """,
            (n,),
...
def topk_neighbours(conn, vector: list[float], k: int, exclude_id: str) -> list[str]:
    with conn.cursor() as cur:
        cur.execute(
            ...
            SELECT id::text
            FROM sowknow.document_chunks
            WHERE embedding_vector IS NOT NULL AND id::text != %s
