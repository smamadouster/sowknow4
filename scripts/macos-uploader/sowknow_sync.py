...
class SyncConfig:
    ...
    def __init__(self):
        self.api_url: str = "https://sowknow.gollamtech.com"
        self.api_token: Optional[str] = None
        self.watch_folders: List[Dict[str, str]] = []
        self.file_types: List[str] = [
            ".pdf", ".docx", ".doc", ".txt", ".md", ".jpg", ".jpeg",
            ".png", ".gif", ".xlsx", ".xls", ".pptx", ".json", ".csv",
            ".webp", ".tiff", ".tif", ".bmp", ".odt", ".ods", ".odp",
            ".html", ".htm", ".xml", ".rtf"
        ]
        self.exclude_patterns: List[str] = [
            ".DS_Store", "Thumbs.db", ".git", "__MACOSX",
            "node_modules", ".venv", "venv", ".Trash"
        ]
...
class FileSyncHandler(FileSystemEventHandler):
    ...
    def on_moved(self, event):
        if not event.is_directory:
            self.pending_files.add(Path(event.dest_path))
    ...
    def sync_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if response.ok:
                ...
                file_hash = self.calculate_file_hash(file_path)
                if file_hash:
                    self.state.mark_uploaded(file_hash, result.get("document_id", result.get("id", "")))
                logger.info(f"Successfully uploaded {file_path.name}")
                return True
            else:
                logger.error(f"Upload failed for {file_path.name}: {response.status_code} {response.text[:200]}")
                return False
...
def install_autouploader():
    """Offer to install the auto-uploader launchd service if available."""
    auto_uploader = Path(__file__).parent / "sowknow-auto-uploader.py"
    if auto_uploader.exists():
        print("\n=== SOWKNOW Auto-Uploader ===")
        print(f"Auto-uploader script found: {auto_uploader}")
        print("To install it as a background service, run:")
        print(f"  python3 '{auto_uploader}' --install")
        print("  python3 '{auto_uploader}' --start-service")
...
def main():
    ...
    # Watch mode (default)
    if args.watch or (not args.sync and not args.setup):
